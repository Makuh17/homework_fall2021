...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        # It looks like the "delta_network", which predicts the normalized difference to the next state, simply takes as
        # input the normalised state and action.
        ...
        #NOTE: the above prediction gets detached, but it should be fine as it is not the output used for GD
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        prediction, _ = self(obs, acs, data_statistics['obs_mean'],data_statistics['obs_std'], data_statistics['acs_mean'],
                          data_statistics['acs_std'], data_statistics['delta_mean'], data_statistics['delta_std'])
        # Note to self, the same as above could be achieved using self(obs, acs, **data_statistics)
        # TODO(Q1) get numpy array of the predicted next-states (s_t+1), DONE above
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        loss = self.loss(pred, ptu.from_numpy(target))  # TODO(Q1) compute the loss, DONE
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
